...
def sample(config):
    ...
    names = sorted(glob.glob(os.path.join(config.source_dir, "*.jpg")))

    random_obj.shuffle(names)
    public = names[:config.public_size]
    private = names[config.public_size:]
...
if __name__ == "__main__":
    ...
    parser.add_argument("public_size", metavar="NUMPUBLIC", type=int)
