...
def discriminator_forward(config, incoming,
    ...
    with tf.variable_scope(scope, name, reuse=reuse):
        output = conv_2d(incoming, 2 * config.gen_dim, 5, strides=2, name="conv1")
        output = leaky_relu(output, 0.2)

        output = conv_2d(output, 4 * config.gen_dim, 5, strides=2, name="conv2")
        output = leaky_relu(output, 0.2)

        output = conv_2d(output, 8 * config.gen_dim, 5, strides=2, name="conv3")
